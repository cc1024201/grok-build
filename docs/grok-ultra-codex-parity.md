...
- Codex: `openai/codex@85e0661c3baacc62db7b006d2b8085b006d0795e`

This document distinguishes **core user-facing collaboration parity** from exact implementation parity. The two products use different model services, so identical client orchestration cannot guarantee identical answer quality.
...
### 1. Model-service reasoning tier — fundamental

Current Codex represents `ultra` as an exact model-facing reasoning effort. Grok Ultra cannot manufacture a Grok server capability that the Grok model catalog does not advertise. It therefore chooses the strongest verified Grok effort (for example `max`, `xhigh`, or `high`) and omits the parameter when support cannot be verified.

This is the largest irreducible difference. It can only disappear if the Grok model service exposes an equivalent tier.

### 2. Per-child spawn controls — meaningful but not required for the basic loop
...
### 3. Agent identity and tree topology — observability and recursive-management gap
...
### 4. Wait semantics — behavioral gap

Codex `wait_agent` waits for agent-mailbox activity and can be interrupted by user steering; the event need not be terminal completion. Grok Ultra currently delegates to Grok Build's `WaitTasks` `WaitAny` path and returns when a selected child reaches a terminal state or the timeout expires.

Live correction remains available through `send_message`, but the wait primitive itself is less expressive.

### 5. Follow-up identity continuity — implementation gap
...
### 6. Interrupt semantics — implementation gap
...
### 7. Message metadata and communication topology — implementation gap
...
### 8. Dedicated UI and telemetry — product gap
...
### 9. Proven outcome parity — still unverified
...
The model-service `ultra` tier is outside the client's control.
